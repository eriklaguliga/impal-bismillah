...
void garis(){
	...
	garis();
	cout<<" \tSelamatDatang Di Aplikasi Peminjaman Inventaris \n";
	garis();
	cout<<endl;
	atas:
...
menu:
	...
	cout<<" 3. Pendaftaran Anggota Peminjam\n";
	cout<<" 4. Data Anggota Peminjam\n";
	cout<<" 5. Peminjaman Barang\n";
	cout<<" 6. Pengembalian Barang\n";
	cout<<" 7. Rekap Barang\n";
	cout<<" 8. Keluar\n";
...
//input data barang
input:
		cout<<" Masukan Banyaknya barang yang akan diinputkan : "; cin>>a;
		for(b=0; b<a; b++){
			cout<<" Masukan data barang ke- "<<b+1<<endl;
			cout<<" Kode barang\t ";
			cin>>z[b].kodeB;  //Input Kode Buku
			cout<<" Nama Barang\t ";
			cin>>z[b].namabarang; //Input Judul Buku
			cout<<" Jumlah Barang\t ";
...
pjm:
	...
	goto menu; //Menuju ke Menu
		...
		cout<<" Kode barang : "; cin>>x[f].kb; //Masukkan kode buku
		for(b=0; b<a; b++){ //nilai awal b=0, b terus bertambah, hingga kurang dari a
			if(z[b].kodeB==x[f].kb){ //Jika kode buku ada pada memori maka
				cout<<" nama barang : "<<z[b].namabarang<<endl; //Output judul buku
				break;
			}else{ //jika tidak ada maka
				cout<<" Data tidak ditemukan "<<endl; //Data tidak ditemukan
...
bali:
	...
	for(f=0; f<e; f++) //nilai f awal 0, f terus bertambah hinga max kurang dari e
		if(x[f].kp==balik){
			cout<<" Kode barang : "<<x[f].kb<<endl; //Menampilkan Kode buku
...
rekap:
		...
		for(j=0; j<e; j++){ //nilai awal j adalah 0, terus bertambah hingga max kurang dari e
			...
			cout<<" Kode Barang : "<<x[j].kb<<endl; //Menampilkan Kode Buku
			for(b=0; b<a; b++){ //nilai awal b adalah 0, terus bertambah hingga kurang dari a
				if(z[b].kodeB==x[j].kb){ //Jika Kode Buku ada dalam memori maka
					cout<<" Nama Barang : "<<z[b].namabarang<<endl; //Menampilkan Judul Buku Sesuai dengan Kode Buku
				}
			}
